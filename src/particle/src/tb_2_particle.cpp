...
/*----Variaveis para o potential fields*/
	...
	//Criando a imagem
	int dimensao_matrix = 5; //significa que a matriz sera quadrada 5x5
	int w_matrix = map.info.width/dimensao_matrix;
	int h_matrix = map.info.height/dimensao_matrix;
...
/*
	...
	for (int i = 0; i < h_matrix; i++){
		for (int j = 0; j < w_matrix; j++){
			...
			map_float_reduce[i*w_matrix + j] = reduce_matrix(float *map_float, int i, int j);
				
		}
	}


	char reduce_matrix(float *distances, int h, int w){
		for (int x = 1; x < dimensao_matrix-1; x++){ //x
			for (int y = 1; y < dimensao_matrix-1; y++){   //y
					return (distances[(y+1)*w + x] + distances[(y-1)*w + x] + distances[y*w + x+1] + distances[y*w + x-1])/4;
				}
			}
		}
	...
	//O Darlan implementou uma matriz temporaria (acredito que nao influencia) e fez a media dos 8 vizinhos e tambem obteve uma interacao na ordem de k.
	//
*/
	...
	cv::circle(Source_Target, destiny, 10, 187, -1);
...
//Convertendo as cordenadas 
	...
	for (int i = 0; i < height_potential_fields; i++){
		for (int j = 0; j < width_potential_fields; j++){
			...
			if (Source_Target_small.at<uchar>(cv::Point(j,i)) == 187){ //destino
	...
	*/
	...
	while(arrive<1000000){
		...
		if(map_float[cy_pf*width_potential_fields + cx_pf] < 0.1){
			...
			if (lowest_index==1){
				cy_pf++;
				cx_pf--;
				potential_fields.at<uchar>(cv::Point(cx_pf,cy_pf)) =255;
			}
